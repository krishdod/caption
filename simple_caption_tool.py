...
def get_captions_text() -> str:
    """Get the text from Windows Live Captions (uses cached text control when possible)."""
    global captions_text_ctrl
    ...
    try:
        with uia.UIAutomationInitializerInThread():
            if captions_text_ctrl is not None:
                try:
                    txt = (captions_text_ctrl.Name or "").strip()
                    if txt:
                        return txt
                except Exception:
                    captions_text_ctrl = None
...
def _wait_for_captions_state(opened: bool, attempts: int = 10, interval: float = 0.025) -> bool:
    """Poll quickly until captions reach the expected open/closed state."""
    ...
    try:
        while is_auto_running:
            try:
                ...
                if updated:
                    root.after(0, lambda t=updated: _on_caption_update(t, trigger_auto_send=True))
                    root.after(
                        0,
